...
// The add-a-card flow, shaped for a sidebar: name autocomplete on top, then
// the printings of the picked name as compact rows. Picking a row asks for
// condition, language, quantity and finish, then every copy is filed into the
// container — a binder's land in stand-by to be dragged into pockets, a box's
// go straight in. Nothing closes afterwards: adding several cards in a row is
// the normal case, so the panel stays where the user left it.
